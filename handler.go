...
func HandleFunc[T, U any](
	handle func(context.Context, T) (U, error),
...
func defaultErrorHandler[T, U any]() func(context.Context, EventWriter[U], Event[T]) {
	return (func(ctx context.Context, w EventWriter[U], event Event[T]) {
		w.Write(NewErr[U](event.Err))
	})
}

func wrapHandle[T, U any](handle Handler[T, U]) Handler[T, U] {
	return func(ctx context.Context, w EventWriter[U], event Event[T]) {
		select {
		case <-ctx.Done():
